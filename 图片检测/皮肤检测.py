...
def saveSkinImg(image, savePath):
    img = cv2.imread(image, cv2.IMREAD_COLOR)
    ycrcb = cv2.cvtColor(img, cv2.COLOR_BGR2YCR_CB)
    (y, cr, cb) = cv2.split(ycrcb)
...
if __name__ == "__main__":
    saveSkinImg('./output/18.png', './save/1.png')
    # ellipse_detect('./6.png') #效果第二
    # cr_otsu('./6.png') #效果最差
    # crcb_range_sceening('./output/11.png') #效果第一 最好
    # hsv_detect('./6.png') #效果第三
